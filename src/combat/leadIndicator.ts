...
// should aim at to hit `targetPos`/`targetVel` with a shot exiting at `projectileSpeed` —
// accounting for the shooter's own velocity, since projectiles inherit it (see weapons.ts). Works
// for a moving shooter and/or moving target. Returns null when no real intercept exists (e.g. the
// target can outrun the projectile).
...
// shooter's actual current nose. Finds the closest approach between the fired projectile's path and
// the target's path, both extrapolated at their CURRENT velocity, bounded by the projectile's
// lifetime, and returns that closest-approach distance.
