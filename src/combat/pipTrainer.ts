...
// opponent ship — see the Evasive Pilot drill (combat/ai/evasiveAI.ts's evasiveThink) for that
// heavier approach, which this intentionally skips. This is just a point mass with capped
// acceleration (so its motion still reads as inertial rather than teleporting) plus a
// hold-to-score timer. Ported from the original project's combat/pipTrainer.ts, reusing this
// project's math/vec.ts helpers (sub/add/scale/length/dot) instead of re-declaring local ones —
// only randomUnitVec3 is genuinely new.
import type { Vec3 } from '../core/types';
import type { ShipBody } from '../core/world';
import { add, clamp, dot, length, normalize, scale, sub } from '../math/vec';
...
// combat/ai/orbiterDrifterAI.ts's OrbitState.center.
...
// Seconds the score-flash ring takes to fade after a rep completes — see render/renderer.ts.
...
export interface PipTrainerState {
  ...
  outcome: 'active' | 'won';
  // The options this run started with — kept on the state itself (a deliberate deviation from the
  // original, which threads a separate opts object through) so main.ts doesn't need a second piece
  // of module-level state to remember which settings are active.
  opts: PipTrainerOptions;
}
...
export function startPipTrainer(player: ShipBody, opts: PipTrainerOptions): PipTrainerState {
  const { forward } = computeAxes(player.quat);
  const anchor = add(player.pos, scale(forward, ANCHOR_DISTANCE));
  return {
    anchor,
    pos: { ...anchor },
    vel: { x: 0, y: 0, z: 0 },
...
function pickNewTargetVelocity(opts: PipTrainerOptions): Vec3 {
  ...
  return scale(randomUnitVec3(), magnitude);
}

export function updatePipTrainer(state: PipTrainerState, player: ShipBody, dt: number): void {
  ...
  const toPipBefore = sub(state.pos, player.pos);
  const toPipBeforeDist = length(toPipBefore);
  ...
  if (avoidRad > 0 && toPipBeforeDist > 1e-6) {
    const alongBoresight = scale(forward, dot(toPipBefore, forward));
    const lateral = sub(toPipBefore, alongBoresight); // component of toPip perpendicular to boresight
    const lateralLen = length(lateral);
    ...
    if (lateralLen < avoidRadiusAtRange) {
      ...
      const fleeDir = lateralLen > 1e-3
        ? scale(lateral, 1 / lateralLen)
        : (() => { const a = Math.random() * Math.PI * 2; return add(scale(right, Math.cos(a)), scale(up, Math.sin(a))); })();
      const overshoot = 1 - clamp(lateralLen / avoidRadiusAtRange, 0, 1); // 1 = dead-on, 0 = at the edge
      state.targetVel = scale(fleeDir, opts.speed * (0.6 + 0.4 * overshoot));
    }
  }
  ...
  if (!avoiding) {
    const offset = sub(state.pos, state.anchor);
    const dist = length(offset);
    if (dist > WANDER_RADIUS) {
      const inward = scale(normalize(offset), -opts.speed);
      const overshoot = clamp((dist - WANDER_RADIUS) / WANDER_RADIUS, 0, 1);
      state.targetVel = add(scale(state.targetVel, 1 - overshoot), scale(inward, overshoot));
    }
  }
  ...
  const dv = sub(state.targetVel, state.vel);
  const dvLen = length(dv);
  const step = maxAccel * dt;
  state.vel = dvLen <= step ? { ...state.targetVel } : add(state.vel, scale(dv, step / dvLen));
  state.pos = add(state.pos, scale(state.vel, dt));

  const toPip = sub(state.pos, player.pos);
  const toPipDist = length(toPip);
  const aimed = toPipDist > 1e-6 &&
    Math.acos(clamp(dot(toPip, forward) / toPipDist, -1, 1)) <= AIM_CONE_RAD;
