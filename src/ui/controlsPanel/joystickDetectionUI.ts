import type { AxisConcept, GamepadSnapshot } from '../../input/actions';
import * as Gamepad from '../../input/gamepad';
import * as Joystick from '../../input/joystickMap';
import { isChromium } from '../deviceDetect';

// Ported from the original project's ui/controlsPanel/joystickDetectionUI.ts.
...
// The single most important vJoy caveat, reused across the tooltip, the banner, and the help box
// so the message is identical everywhere: on Chrome/Edge, vJoy doesn't just read zero — its mere
...
// A one-line health readout of the browser's gamepad access, so a user whose stick won't appear
// can see *why* at a glance instead of just an empty list.
function renderDiagnostics(): void {
  const d = Gamepad.getDiagnostics();
  if (!d.supported) {
...
export function initJoystickDetectionUI(): void {
  ...
  // Refresh immediately whenever a device connects/disconnects (Chrome can stay blind under pure
  // polling until a connect event fires), on top of the interval below.
  Gamepad.initConnectionListeners(() => {
    if (panel.style.display !== 'none') renderGamepads();
  });
  // live poll while the panel is open — the browser gives us no reliable per-device change event,
  // and detection depends on the user actually moving/pressing each stick, so this needs to keep checking
  setInterval(() => {
