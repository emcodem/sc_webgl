...
// Maps each digital action to the joystick axis concept it corresponds to (if any), so the
// bindings table can show keyboard and joystick side by side per control. decoupleToggle/
// spaceBrake/boost/primaryFire/interact have no analog equivalent — those get a joystick BUTTON
...
// Joystick.read() uses shorthand keys that differ from the axisMap concept names above
const STICK_KEY_FOR_CONCEPT: Record<AxisConcept, keyof ReturnType<typeof Joystick.read>> = {
...
// one roll axis covers rollLeft AND rollRight), so unlike keyboard rebind (one key per action)
// this listens for whichever axis moves and binds the whole concept to it. Detects the exact
// device + axis array-index live, so unlike XML-imported axis bindings (which guess the index
// from a letter), a manually-captured binding needs no guessing at all.
// =====================================================================
...
function pollAxisRebind(): void {
  ...
  Gamepad.poll();
  const current = Gamepad.getSnapshot();
...
// Escape cancels an in-progress axis capture (separate from the keyboard rebind Escape handler
// below, which only fires when pendingRebindAction is set). Stops propagation so it doesn't also
// trigger the panel's own Escape-to-close handler.
...
// =====================================================================
// Joystick BUTTON rebind capture — for discrete toggles rather than analog axes. A button binding
// is a plain rising edge: any button not already held at capture start that becomes pressed is
// the one bound. No direction pairing needed (one action, one button).
// =====================================================================
...
function completeButtonRebind(action: ActionName, pad: GamepadSnapshot, buttonIndex: number): void {
  ...
  const niceName = pad.id.split('(')[0].trim();
  const ref = { vid: pad.vid, pid: pad.pid, axisCount: pad.axesValues.length, buttonCount: pad.buttonsPressed.length };
  Joystick.bindButton(action, { ...ref, buttonIndex, label: `${niceName} button[${buttonIndex}]` });
  rebindStatus.textContent = `Bound "${niceName}" button [${buttonIndex}] to ${Keybinds.getActionLabels()[action]}.`;
  cancelButtonRebind();
  renderBindings();
}
...
function startButtonRebind(action: ActionName): void {
  ...
  Gamepad.poll();
  buttonRebindBaseline = Gamepad.getSnapshot().map(p => ({ index: p.index, buttons: p.buttonsPressed.slice() }));
  rebindStatus.textContent = `Press the joystick button for "${Keybinds.getActionLabels()[action]}"… (Esc to cancel)`;
  renderBindings();
  buttonRebindRAF = requestAnimationFrame(pollButtonRebind);
}
...
// =====================================================================
// Mouse BUTTON rebind capture — same one-action-one-button idea as the joystick button capture
// above, but there's no live device to poll: a mouse click is a discrete DOM event, so capture is
// a single one-shot 'mousedown' listener (capture phase, so it intercepts the click before it can
// also request pointer-lock capture or reach game input) rather than a per-frame poll loop.
// =====================================================================
...
// Pitch/yaw are always driven by mouse-look (absolute virtual-stick deflection), additively with
// keyboard/joystick, so there's no separate discrete mouse *button* to bind for them — the Mouse
// column just says so instead of offering a bind button.
...
function onMouseRebindCapture(e: MouseEvent): void {
  ...
  MouseButtons.bindMouseButton(action, { button: e.button, label: MouseButtons.mouseButtonLabel(e.button) });
  rebindStatus.textContent = `Bound "${MouseButtons.mouseButtonLabel(e.button)}" to ${Keybinds.getActionLabels()[action]}.`;
  cancelMouseRebind();
  renderBindings();
}
...
function startMouseRebind(action: ActionName): void {
  ...
  rebindStatus.textContent = `Click a mouse button for "${Keybinds.getActionLabels()[action]}"… (Esc to cancel)`;
  renderBindings();
...
export function renderBindings(): void {
  ...
  const bindings = Keybinds.getBindings();
  const stick = Joystick.read();
  const axisMap = Joystick.getAxisMap();
  const buttonMap = Joystick.getButtonMap();
  const mouseButtonMap = MouseButtons.getMouseButtonMap();

  let html = '<table id="ctrl-bindings-table">' +
    '<colgroup><col class="ctrl-col-action"><col class="ctrl-col-kb"><col class="ctrl-col-mouse"><col class="ctrl-col-joy"></colgroup>' +
    '<thead><tr>' +
  ...
  for (const action of Object.keys(labels) as ActionName[]) {
    ...
    const chordLabel = hasChord
      ? `<span class="ctrl-found">${chords.map(Keybinds.chordToLabel).join(' / ')}</span>`
    ...
    if (concept) {
      ...
      const inverted = Joystick.getInvert()[concept];
      const invertHtml = (binding && !isListening)
        ? `<label style="display:inline-flex; align-items:center; gap:4px; margin-left:8px; color:rgba(143,211,199,0.6)">` +
      ...
      if (isListening) {
        ...
        const detected = !!Gamepad.findDevice(binding);
        valueHtml = detected
  ...
  bindingsList.querySelectorAll('.ctrl-kb-unbind-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      const action = btn.getAttribute('data-action') as ActionName;
      Keybinds.setBinding(action, []);
      rebindStatus.textContent = `Unbound keyboard key for "${Keybinds.getActionLabels()[action]}".`;
      renderBindings();
    });
  });
  ...
  bindingsList.querySelectorAll('.ctrl-mouse-unbind-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      const action = btn.getAttribute('data-action') as ActionName;
      MouseButtons.unbindMouseButton(action);
      rebindStatus.textContent = `Unbound mouse button for "${Keybinds.getActionLabels()[action]}".`;
      renderBindings();
    });
  });
  ...
  bindingsList.querySelectorAll('.ctrl-button-unbind-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      const action = btn.getAttribute('data-action') as ActionName;
      Joystick.unbindButton(action);
      rebindStatus.textContent = `Unbound joystick button for "${Keybinds.getActionLabels()[action]}".`;
      renderBindings();
    });
  });

  // one invert checkbox per axis concept, but two actions share a concept (e.g. pitchUp/pitchDown
  // both -> 'pitch') so re-render on change to keep both rows in sync
...
// Keeps the table in sync whenever a control preset is loaded/imported/restored, without the
// preset UI needing to know keybinds/joystick bindings exist.
...
export function initBindingsTableUI(): void {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    Keybinds.setBinding(pendingRebindAction, [[e.code]]);
    rebindStatus.textContent = `Set "${Keybinds.getActionLabels()[pendingRebindAction]}" to ${Keybinds.chordToLabel([e.code])}.`;
    pendingRebindAction = null;
    renderBindings();
