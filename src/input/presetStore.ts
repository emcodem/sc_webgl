import { deserializeAllConfig, serializeAllConfig } from './configRegistry';
...
// Ported from the original project's input/presetStore.ts.
// =====================================================================
...
// Presets saved before joystick/mouse settings joined the schema stored the raw keybind map at
// the top level (no wrapper). Recognize that shape and migrate it in place instead of silently
// discarding old presets.
...
// Called once at startup — silently re-applies whichever preset was active last session, if any.
// A stale reference (e.g. the preset was deleted elsewhere) is cleared rather than retried on
// every load.
