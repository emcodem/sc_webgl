...
// Ported verbatim from the original project (starcitizen_flightsim/src/math/quaternion.ts).
// LOAD-BEARING INVARIANT: ship attitude is quaternion-only, integrated in the BODY frame, so
// angular velocity is always relative to the ship's CURRENT orientation. Never reintroduce Euler
// storage — it caused inverted-feeling pitch after a roll.
...
// base vectors match the original sandbox convention (forward=+Z, right=+X, up=-Y).
// The render layer converts this basis into a three.js camera orientation explicitly (see
// render/camera.ts) rather than assuming three.js's own +Y-up / -Z-forward convention.
...
// (computeAxes maps base up (0,-1,0) to `up`, so the matrix's up column is -up).
...
// Turns `current` toward `target` by at most `maxAngleRad`, snapping to `target` once within
// range — a generic "turn at a capped rate" primitive (used by the turret behavior's tracking).
...
export function slerp(a: Quat, b: Quat, t: number): Quat {
  ...
  if (dot > 0.9995) {
    return quatNormalize({
      x: a.x + t * (bb.x - a.x), y: a.y + t * (bb.y - a.y),
      z: a.z + t * (bb.z - a.z), w: a.w + t * (bb.w - a.w)
    });
  }
  ...
  return quatNormalize({
    x: s0 * a.x + s1 * bb.x, y: s0 * a.y + s1 * bb.y,
    z: s0 * a.z + s1 * bb.z, w: s0 * a.w + s1 * bb.w
  });
}
