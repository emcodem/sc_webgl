...
// Ported verbatim from the original project's scenarios/gatePath.ts. Generates a helical "fly
// through these rings" course along +Z — a corkscrew translation that approximates a barrel roll's
// flight path (looping laterally while still progressing forward). Following the gates in order
// requires actually flying the maneuver, not just pointing at a single target. All gates share one
// forward-facing orientation since the course only ever progresses along +Z.
