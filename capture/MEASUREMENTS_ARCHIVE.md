# ARCHIVE — superseded 2026-07-26 by a lean MEASUREMENTS.md

**This file is a historical record, not the source of truth.** As of 2026-07-26, `MEASUREMENTS.md`
was rewritten as a lean table of values with a CONFIRMED/SUSPECT status column; the narrative
reasoning, dated session log, and full derivation detail that used to live there was moved here
verbatim instead of being deleted, so nothing hard-won is lost. **Trigger:** a session that day found
engine power-triangle allocation is a real, previously-untracked confound on thrust/rotation output
(see `MEASUREMENTS.md`'s "⚠ CRITICAL: power triangle allocation" section) — since power allocation
was never tracked or controlled in ANY session whose results appear below, essentially every
rotational/thrust-based number in this file should be treated as **suspect pending re-verification
at confirmed full engine power**, not as settled ground truth. Pure input-mapping findings (deadzone
%, curve exponent, accumulator clamps, VJoyAnglePilots being cosmetic-only) are NOT thrust-dependent
and are carried into the new `MEASUREMENTS.md` as CONFIRMED.

Some cross-references elsewhere in this repo (`BOOST_FINDINGS.md`, `handoff.md`, `SESSION_NOTES.md`,
etc.) point at specific dated sections "in MEASUREMENTS.md" that now only exist here — if a link
doesn't resolve in the new file, look for it in this archive instead.

Below is the original file, unedited from just before the 2026-07-26 restructuring.
...
## ⚠ CRITICAL (2026-07-26): power triangle allocation is a real, previously-untracked confound

A boosted-pitch 360°-sustained-hold test landed almost exactly on the coded 82°/s rate (0.166°
residual after 2 full laps) only once the power triangle was set to **full power to engines**. The
SAME test shortly before, at whatever default/unrecorded power allocation was active, read a rate
~7-9°/s slower (residual ~36.6° after just 1 lap at the same nominal-rate-derived duration). Power
allocation was never tracked or controlled for in any capture session before this one — see
`settings_checklist.md`'s new "Power triangle" item. **Practically every existing boosted-pitch
measurement in this file (the spool-up rate_ss=70.15-76.69 fits, the afterburner-ratio rows, likely
more) was captured without confirming full engine power, and should be treated as suspect —
possibly reading low for this reason rather than reflecting a real sub-coded afterburner effect —
until re-verified with power confirmed full.** Non-boosted / non-pitch measurements may be less
affected (thruster headroom matters more under boost, when demand is highest) but haven't been
checked either. See "Pitch UP boosted, full engine power" (2026-07-26) below for the confirming data.
...
**⚠ Dispute raised 2026-07-25, RESOLVED for both pitch directions 2026-07-26.** Building a reusable
version of this fit (`analysis/fit_spool_response.py`) initially found it couldn't reproduce the
DOWN-boosted numbers above on a ~0.55-0.6s dwell window (degenerate near-critically-damped fits
regardless of fitting domain). Turned out to be a window-length problem, not a domain or method
problem: a longer dwell (1.1s), long enough to show a full overshoot-THEN-undershoot cycle before the
landmark exits frame, fits cleanly for BOTH pitch UP (ωₙ=8.14 rad/s, ζ=0.71, rate_ss=76.69) and pitch
DOWN (ωₙ=8.28 rad/s, ζ=0.82, rate_ss=70.15) — both land inside this table's ~8.0-8.6 rad/s range with
2nd-order beating 1st-order by ~3.7-4.0×, strong evidence the original fits were sound and this
session's earlier short-window failures were the real anomaly. Yaw and the non-boosted-pitch rows
haven't been re-verified at a matching longer dwell, so treat those as *plausible, not re-confirmed*.
See "Pitch DOWN boosted spool-up — matching longer-dwell redo" (2026-07-26) for the full UP vs DOWN
comparison, including a new finding (DOWN is meaningfully more damped than UP, ζ 0.82 vs 0.71).
...
### Pitch UP boosted spool-up rise curve — new reusable fit tool, and a fitting-method problem found — 2026-07-25

Captured the one rise curve still missing from the 2nd-order picture above (pitch UP, boosted) —
same method as the DOWN/yaw curves: `mouse_hold_capture.py --axis pitch --offsets 0,-1080 --boost
--dwell 0.55 --ramp 0.05` (0.05s ramp approximating a step), sun manually positioned near the top of
frame first (~1920,184, read from the capture's own frame 0, giving the sun room to sweep down).
Boosted counter-maneuver (`--offsets 0,1080 --boost`, same dwell/ramp) applied after, confirmed via
screenshot. Built a new committed tool for this, `capture/analysis/fit_spool_response.py` (the
2nd-order fit for the other three conditions above was done ad hoc, not saved as reusable code).

**The new tool surfaced a real problem with the fitting method itself, not just this one capture.**
Fitting the raw differentiated rate curve directly (as the original ad hoc fits apparently did)
repeatedly converges to a degenerate near-critically-damped solution (ζ pinned at the 0.999 bound,
ωₙ 44-54 rad/s — nothing like the ~8 rad/s reported above) with a poor RMS (~10-12°/s), on BOTH this
new UP capture and, when re-run against the already-recorded DOWN-boosted clip
(`pitch-spoolup-boost-20260723-232357`) that produced the rate_ss=75.75/ωₙ=8.009/ζ=0.916 result
quoted above, the same degeneracy. Switching to fit against the integrated ANGLE curve instead (less
sensitive to per-frame tracking jitter) fixed the numerical instability (RMS well under 1°) but
traded it for a worse problem: printing the fitted models' own implied rate curve back out and
comparing it to the observed rate curve shows both the 1st- and 2nd-order angle-domain fits flatten
to a constant within ~0.2s and completely miss the real, visible overshoot (UP: observed peaks at
76.23°/s at t=0.405s, decaying back to ~39°/s by t=0.57s) — the angle domain barely penalizes this
because the overshoot's extra area is nearly cancelled by the subsequent undershoot's area deficit.

**Net effect: neither fitting domain is trustworthy for ωₙ/ζ on a short (~0.6s), single-overshoot
window** — rate-domain is numerically unstable, angle-domain is blind to the exact feature that
distinguishes 1st- from 2nd-order dynamics. This calls into question whether the rate_ss/ωₙ/ζ numbers
in the table above (all four conditions, fit the same ad hoc way) are as solid as presented — they
may need to be re-derived once a fitting method that actually reproduces the visible overshoot shape
exists, most likely via a longer dwell (enough for the curve to complete at least one full
oscillation and give the fit something to anchor ωₙ/ζ against) and/or residual weighting that
penalizes the overshoot region more. Not attempted yet.

**What IS trustworthy from this session (model-free, directly observed):**

| condition | peak observed rate | time of peak (from step onset) |
|---|---|---|
| pitch DOWN, boosted (re-read from the existing clip) | 69.94°/s | 0.305s |
| pitch UP, boosted (new capture) | 76.23°/s | 0.405s |

UP's peak (76.23°/s) sits closer to the API's 82°/s than DOWN's peak (69.94°/s) does, and is close to
the previously-quoted (now-questioned) DOWN rate_ss of 75.75. This is consistent with — though doesn't
independently re-confirm, given the fitting-method doubt above — the existing conclusion that the
short-hold boosted-UP reps (66.98/62.49) under-read and the true boosted-UP rate is nearer the API
value than those short-hold numbers suggested. Single rep each direction; peak timing/magnitude can
still carry the same rep-to-rep noise already established for boosted-pitch measurements generally
(~6-7%).

### Pitch UP boosted spool-up — RESOLVED with a longer dwell, disputed status lifted — 2026-07-26

The disputed status above turned out to be a **window-length problem, not a fitting-domain
problem.** Re-captured pitch UP boosted with `--dwell 1.1` instead of `0.55` (same 0.05s ramp, same
seed method) — long enough that the rate curve completes a full overshoot-THEN-undershoot cycle
before the landmark exits frame (confirmed via `peak_brightness`: steady ~193-217 through the clean
rise/overshoot/undershoot, then an abrupt, erratic swing — 169, then spiking to 209-217 — starting
around t≈0.72s after step onset, the tell that lock is degrading before the sun fully exits frame
around t≈0.85s). Fit against the pre-contamination window only
(`fit_spool_response.py --trim-end`, `--t0` — see below):

```
python analysis/fit_spool_response.py data/indicator/pitch-spoolup-boost-up-longdwell-20260726-102109 \
    --axis y --seed-x 1920 --seed-y 163 --fov 116 --resolution 3840x2160 --t0 4.517 --trim-end 0.683
```

**rate_ss = 76.69°/s, ωₙ = 8.135 rad/s, ζ = 0.714, RMS 0.138° (2nd-order) vs 0.506° (1st-order, RMS
ratio ~3.7×).** ωₙ lands right inside the original four-condition table's ~8.0-8.6 rad/s range, and
the RMS ratio matches the original "2-4× better" claim — both are strong evidence the *original*
2026-07-23 ad hoc fits were likely sound, and this tool's own earlier short-window failures (on both
this new UP capture and, when re-tried, the existing DOWN clip) were the real anomaly: a ~0.55-0.6s
dwell only ever shows the initial rise and overshoot, never the return swing, so there is nothing in
the data to anchor ωₙ/ζ against — angle-domain fitting on such a window quietly converges to a flat,
near-critically-damped curve whose RMS looks fine only because the missing undershoot's area deficit
happens to cancel the overshoot's extra area. **Practical rule going forward: always capture enough
dwell for the rate to visibly turn back down (or up) after its first overshoot before trusting any
wn/zeta output** — a fit that only sees a single rise-and-peak is not reliable no matter how low its
own reported RMS looks.

Two new tool options used above, both now part of `fit_spool_response.py`: `--trim-end` (cut the
analysis window short of the hold's nominal end, for when the landmark starts approaching the frame
edge before the hold is technically over) and `--t0` (manual override for `find_alignment`'s
correlation search, which got measurably less reliable on this longer/noisier trace — align_corr
dropped to 0.815 from the ~0.95+ seen on short clean captures, and auto-alignment landed the window
~0.09s off from the true onset read by eye off the printed trace).

Boosted counter-maneuver applied after (`--offsets 0,1080 --boost --dwell 1.1`, matching magnitude/
duration) — this one didn't fully restore the sun to frame (some residual attitude drift, likely the
same ~3-9% UP/DOWN rate asymmetry already documented elsewhere), left alone since it's cosmetic only
in a private AC map.

**Still open (superseded below):** this is one rep. The DOWN-direction condition hasn't been
re-captured at a matching longer dwell yet, so its own rate_ss=75.75/ωₙ=8.009/ζ=0.916 (and the
yaw/non-boosted-pitch rows in the original table) remain *plausible but not re-verified* by this
method — they were likely fine per the reasoning above, but "likely fine" isn't the same as
"re-confirmed with a full-cycle window."

### Pitch DOWN boosted spool-up — matching longer-dwell redo — 2026-07-26

Same method, opposite direction: `mouse_hold_capture.py --axis pitch --offsets 0,1080 --boost
--dwell 1.1 --ramp 0.05`, sun manually positioned ~15° below center first (giving ~57° of clean
travel room toward the top edge — less margin than UP's near-edge start, but still comfortably more
than the ~50-60° actually needed). This curve turned out messier than UP's: a clean single
overshoot+undershoot cycle through about t=0.75-0.80s after step onset, but then a second, slightly
*larger* negative peak around t=0.85s before a real (not just tracking-artifact) rapid rate collapse
around t=0.90-0.95s — `peak_brightness` stays rock-steady (~195-221) through t≈1.42s of hold time
(video t=5.417) and only actually craters (203→162→54) starting at t≈0.92-0.93s after onset, so the
crash in the rate itself very likely reflects something real (possibly the ship crossing some other
governor/limit at extreme deflection, not investigated further) rather than a lost-lock artifact —
but it does mean the usable window is bounded by that crash, not by tracking failure.

Swept `--trim-end` from 0.55 to 0.98s to find where the fit stops being reliable:

| trim_end (s) | rate_ss | ωₙ (rad/s) | ζ | RMS (2nd-order) |
|---|---|---|---|---|
| 0.55-0.70 | 75-76 | 8.46-8.64 | 0.999 (pinned — degenerate) | 0.12 |
| 0.75 | 73.01 | 8.527 | 0.930 | 0.125 |
| 0.80 | 71.74 | 8.436 | 0.883 | 0.127 |
| 0.85 | 70.88 | 8.360 | 0.850 | 0.128 |
| **0.90** | **70.15** | **8.283** | **0.820** | **0.131** |
| 0.92 | 69.65 | 8.224 | 0.798 | 0.142 |
| 0.95 | 65.28 | 7.692 | 0.618 | 0.475 (jumps — contamination) |

Below trim_end≈0.75 the fit hits the same degenerate ζ≈0.999 boundary the original short-window UP
attempts did (not enough oscillation yet to constrain damping); ωₙ stays remarkably stable (8.2-8.6)
across the whole clean range regardless of exactly where the window ends, which is itself a good
robustness signal — RMS jumps sharply at 0.95, confirming trim_end=0.90 is right at the edge of the
genuinely clean data. **Reporting trim_end=0.90 as the primary result: rate_ss=70.15°/s, ωₙ=8.283
rad/s, ζ=0.820, RMS 0.131° (2nd-order) vs 0.527° (1st-order, ratio ~4.0×).**

**UP vs DOWN comparison, both via the identical longer-dwell method:**

| direction | rate_ss | ωₙ (rad/s) | ζ | 2nd/1st RMS ratio |
|---|---|---|---|---|
| UP | 76.69°/s | 8.135 | 0.714 | ~3.7× |
| DOWN | 70.15°/s | 8.283 | 0.820 | ~4.0× |

ωₙ agrees within ~2% between directions (8.14 vs 8.28) and both sit inside the original table's
~8.0-8.6 rad/s range — strong cross-validation of that shared-natural-frequency finding. rate_ss is
~9% higher for UP, matching the direction (if not quite the magnitude) of the small UP-faster-than-
DOWN asymmetry already documented for non-boosted pitch (~3.2%). ζ is meaningfully higher for DOWN
(0.82 vs 0.71, i.e. DOWN is more damped/less oscillatory) — a new finding, not previously flagged;
single rep each direction, so treat the exact gap as provisional, but the direction (DOWN more
damped) is consistent with `BOOST_FINDINGS.md`'s already-noted "boost increases pitch damping, decreases
yaw damping" pattern extending to a UP/DOWN split too.

Compared against the original (pre-dispute) DOWN number (rate_ss=75.75, ωₙ=8.009, ζ=0.916): this
redo's rate_ss (70.15) and ωₙ (8.283) land within ~7% and ~3% of it respectively — reasonably close,
not exact — while ζ differs more (0.820 vs 0.916, ~10%). Given the demonstrated sensitivity of ζ to
exactly how much data trim_end includes (see table above), that gap is plausibly just where each
capture's clean-data cutoff happened to fall, not necessarily a real discrepancy.

Boosted counter-maneuver applied after (`--offsets 0,-1080 --boost --dwell 1.1`) — attitude
restoration was imperfect this time too (unrelated stray key input mid-session), left alone per the
same cosmetic-only reasoning as UP's.

**Gap #1 status: both pitch directions now resolved to the same standard** (longer-dwell capture,
full-cycle fit, `fit_spool_response.py --t0/--trim-end`). Yaw and non-boosted pitch rows in the
original four-condition table remain unverified at a matching longer dwell.

**⚠ UPDATE 2026-07-26, later same night: see "Power triangle allocation is a real,
previously-untracked confound" at the top of this file.** Neither of the two captures above had
engine power allocation confirmed/recorded — both rate_ss values here should be treated as suspect
pending a full-engine-power re-capture, not as final.

### Pitch UP boosted, full engine power — 360°-sustained-hold, nominal-rate reference — 2026-07-26

Prompted by suspicion that the spool-up fits above were reading low: instead of another rise-curve
capture, tested the STEADY-STATE rate directly via the 360°-sustained-hold method (see below),
anchored on the coded/API nominal rate (82°/s boosted) rather than a fitted rate_ss, specifically to
check whether the *coded* value is actually reachable — and whether power allocation matters.

Method: `mouse_hold_capture.py --axis pitch --offsets <target> --boost --dwell <T> --ramp 0.05`
(single-offset list — no leading `0,`, see gotcha below), sun centered first, hold for
`T = n_laps × 360 / 82` seconds, then compare the sun's screen position before vs. after directly
(live screenshot / `analysis.track_landmark.centroid_in_window` on the frame-0 and after images —
continuous tracking through the sweep isn't trustworthy here, matching the existing "360-test needs
verified landmark ID" lesson).

**Tool gotcha found and fixed along the way:** `mouse_hold_capture.py --offsets 0,<target> --dwell
T` holds **every** offset in the list for the full `--dwell T`, not just the last one — fine for the
short spool-up captures above (a harmless sub-second prefix at offset 0), but for a multi-second 360
test it means an entire extra `T` seconds of boost held at zero pitch input before the real hold even
starts, wasting real time and boost-meter charge for nothing. Fix: pass a single offset with no
leading `0,` (`--offsets -1080`, not `--offsets 0,-1080`) — this ramps directly from the ship's
current attitude to the target and holds only once.

| rep | power allocation | n_laps | T (s) | residual | implied rate |
|---|---|---|---|---|---|
| 1 | not confirmed/recorded (session default) | 1 | 4.390 | ~36.6° (rough, eyeballed) | ~73.7°/s |
| 2 | **full power to engines**, Coupled confirmed | 2 | 8.780 | **0.166°** (precise, `centroid_in_window`) | **~81.98°/s** |

Rep 2's implied rate (81.98°/s) matches the coded/API nominal (82°/s) almost exactly. Rep 1, at
whatever power allocation was active by default, reads meaningfully slower. The two results are NOT
reconcilable under a single constant-rate model (a real ~73.7°/s rate would produce a ~73° residual
after 2 laps, roughly double rep 1's shortfall — not the observed 0.166°), so this isn't just
rep-to-rep noise: something changed between the two captures, and full engine power is the one
deliberate, known change. **This is the strongest evidence yet that this project's boosted-pitch
rate is fully consistent with the coded 82°/s — provided the engines actually have full power.**
Single comparison; the reasoning is solid but this should be treated as an initial finding, not
a fully repeated result — a repeat at full power (same n_laps/T) would firm this up further.
