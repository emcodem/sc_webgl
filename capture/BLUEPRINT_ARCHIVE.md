# ARCHIVE — superseded 2026-07-26 by a lean, pure-methodology BLUEPRINT.md

**This file is a historical record, not the source of truth.** As of 2026-07-26, `BLUEPRINT.md` was
rewritten as a pure how-to-measure guide; the dated findings log, open-items list, and per-result
narrative that used to live there was moved here verbatim instead of being deleted. Measured values
now live in `MEASUREMENTS.md` as a table (CONFIRMED/SUSPECT status per row); the reasoning/derivation
behind any of them is in `MEASUREMENTS_ARCHIVE.md`. See `MEASUREMENTS.md`'s "⚠ CRITICAL: power
triangle allocation" section for why essentially every rate/thrust value below (device-path roll/
yaw/pitch reversal fits, 360°-checks, dedicated spool-up readings) should be treated as suspect
pending re-verification at confirmed full engine power — that confound was never tracked in any
session whose results appear below.
